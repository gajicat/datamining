...
from tensorflow.keras.layers import BatchNormalization, MaxPool2D, AveragePooling2D
...
def inception_block_1a(X):
    ...
    X_pool = MaxPool2D(pool_size=3, strides=2, data_format='channels_last')(X)
...
def inception_block_1c(X):
    ...
    X_pool = MaxPool2D(pool_size=(3, 3), strides=(2,2), data_format='channels_last')(X)
...
def faceRecoModel(input_shape):
    ...
    X = MaxPool2D((3, 3), strides = (2,2), data_format='channels_last')(X)
    ...
    # Zero-Padding + MAXPOOL
    X = ZeroPadding2D((1, 1), data_format='channels_last')(X)
    X = MaxPool2D(pool_size = (3, 3), strides = (2, 2), data_format='channels_last')(X)
